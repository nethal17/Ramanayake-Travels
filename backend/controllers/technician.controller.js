...
export const deleteTechnician = async (req, res) => {
    try {
        ...
        // Find the technician with populated userId
        const technician = await Technician.findById(id).populate('userId');
        if (!technician) {
            return res.status(404).json({
                success: false,
        ...
        if (technician.certification && technician.certification.certificateImage) {
            const certPath = path.join(process.cwd(), 'uploads', technician.certification.certificateImage);
        ...
        await User.findByIdAndDelete(technician.userId._id);
...
export const updateTechnicianAvailability = async (req, res) => {
    try {
        ...
        console.log('Update availability request:', { technicianId: id, isAvailable });
        ...
        res.status(200).json({
            success: true,
            message: `Technician is now ${isAvailable ? 'available' : 'unavailable'}`,
            data: technician
        });
...
export const getTechnicianProfile = async (req, res) => {
    try {
        ...
        const technician = await Technician.findOne({ userId }).populate('userId', 'name email phone isVerified');
